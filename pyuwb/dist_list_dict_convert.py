from .client_id_utils import client_id_get_no,pack_client_id
...
def dist_list2dict(dist_list, group_id=1):
    ...
    :param group_id:3
    :return:{1-1-0:1, 1-2-0:2, 1-2-1:3,...}
    """
    ...
    for n in range(len(dist_list)):
        if n==0:
            one_client_id = pack_client_id(group_id, 1, n)
        else:
            one_client_id = pack_client_id(group_id, 2, n-1)
